...
public class StatisticsViewModel : BaseViewModel
{
    ...
    public uint Users { get => _users; set => this.RaiseAndSetIfChanged(ref _users, value);}
    public uint Translators { get => _translators; set => this.RaiseAndSetIfChanged(ref _translators, value); }
    public ulong SongsPlayed { get => _songsPlayed; set => this.RaiseAndSetIfChanged(ref _songsPlayed, value);}
    public ulong XpEarned { get => _xpEarned; set => this.RaiseAndSetIfChanged(ref _xpEarned, value);}
    public uint CommunityLevel { get => _communityLevel; set => this.RaiseAndSetIfChanged(ref _communityLevel, value);}
    public ulong XpLeft { get => _xpLeft; set => this.RaiseAndSetIfChanged(ref _xpLeft, value);}
    public int BeatmapsTracked { get => _beatmapsTracked; set => this.RaiseAndSetIfChanged(ref _beatmapsTracked, value);}
    public float MbUsed { get => _mbUsed; set => this.RaiseAndSetIfChanged(ref _mbUsed, value);}
    public string PlayerAge { get => _playerAge; set => this.RaiseAndSetIfChanged(ref _playerAge, value);}
    ...
    {
        TimeSpan timeSince = DateTime.Now.Subtract(new DateTime(2017, 11, 1));
        
        DateTime time = DateTime.MinValue + timeSince;

        // note: MinValue is 1/1/1 so we have to subtract...
        int years = time.Year - 1;
    ...
    {
        ApiStatisticsV3Model statistics = await ApiAsync.GetApiStatistics();
